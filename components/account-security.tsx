...
type LoginEvent = { id:string; outcome:string; user_agent_summary:string|null; suspicious:boolean; created_at:string };
...
export function AccountSecurity(){
  ...
  async function load(){try{const headers=await authorization();await parseApiResponse(await fetch("/api/account/security",{method:"POST",headers}));const data=await parseApiResponse(await fetch("/api/account/security",{headers,cache:"no-store"}));setSessions(Array.isArray(data.sessions)?data.sessions as Session[]:[]);setEvents(Array.isArray(data.events)?data.events as SecurityEvent[]:[]);setLogins(Array.isArray(data.logins)?data.logins as LoginEvent[]:[]);setNotice("");}catch(reason){setNotice(reason instanceof Error?reason.message:"Account security could not load.");}}
  useEffect(()=>{void load();},[]);
  ...
  return <section className="mt-10 rounded-3xl border border-white/10 bg-white/[.025] p-6 sm:p-8">
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-3"><div><div className="flex items-center gap-2"><Laptop size={19} className="text-amber-300"/><h3 className="font-black">Tracked sessions</h3></div><div className="mt-3 space-y-2">{sessions.map(session=><article key={session.id} className="rounded-xl border border-white/10 p-4"><div className="flex justify-between gap-3"><b>{session.user_agent_family||"Unknown browser"}</b><span className={`text-xs font-black uppercase ${session.revoked_at?"text-white/30":session.current?"text-emerald-300":"text-amber-200"}`}>{session.revoked_at?"revoked":session.current?"this session":"active"}</span></div><p className="mt-1 text-xs text-white/35">Last seen {new Date(session.last_seen_at).toLocaleString()}</p></article>)}{!sessions.length&&!notice&&<p className="text-sm text-white/35">No tracked sessions yet.</p>}</div></div>
      <div><div className="flex items-center gap-2"><History size={19} className="text-sky-300"/><h3 className="font-black">Successful sign-ins</h3></div><div className="mt-3 space-y-2">{logins.map(login=><article key={login.id} className="rounded-xl border border-white/10 p-4"><div className="flex justify-between gap-3"><b>{login.user_agent_summary||"Unknown browser"}</b>{login.suspicious&&<span className="text-xs font-black uppercase text-amber-200">new device</span>}</div><p className="mt-1 text-xs text-white/35">{new Date(login.created_at).toLocaleString()}</p></article>)}{!logins.length&&!notice&&<p className="text-sm text-white/35">No recorded sign-ins yet.</p>}</div></div>
