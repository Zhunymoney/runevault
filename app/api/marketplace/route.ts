...
export async function GET(request: Request) {
  ...
  const url = new URL(request.url), search = (url.searchParams.get("q") ?? "").trim().replace(/[,*()]/g, "").slice(0, 100), category = (url.searchParams.get("category") ?? "").replace(/[^a-zA-Z0-9_-]/g, "").slice(0, 80);
