...
export default function AccountPage() {
    ...
    void Promise.all([getCurrentProfile(), getMyOrders()])
      .then(([currentProfile, currentOrders]) => {
        ...
        if (currentProfile) {
          ...
          setEmailUpdates(
            currentProfile.notification_preferences?.email !== false,
          );
        }
        void getSavedCharacters()
          .then(setCharacters)
          .catch(() =>
            setMessage(
              "Account profile migration is required before saved characters can load.",
            ),
          );
        void getMyRewards()
          ...
          .catch(() =>
            setMessage(
              "Rewards will appear after the rewards migration is applied.",
            ),
          );
        void loadReferrals();
      })
  ...
  // Initial account bootstrap; referral refreshes are invoked explicitly after claims.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  async function referralHeaders(){const{data}=await createClient().auth.getSession();if(!data.session?.access_token)throw new Error("Sign in to use referrals.");return{"Content-Type":"application/json",Authorization:`Bearer ${data.session.access_token}`};}
  async function loadReferrals(){try{const data=await parseApiResponse(await fetch("/api/rewards/referrals",{headers:await referralHeaders(),cache:"no-store"}));setReferralCode(typeof data.code==="string"?data.code:"");setReferrals(Array.isArray(data.referrals)?data.referrals as Referral[]:[]);}catch{/* Rewards migration messaging is already shown by the account loader. */}}
  async function claimReferral(){setSaving(true);setMessage("");try{await parseApiResponse(await fetch("/api/rewards/referrals",{method:"POST",headers:await referralHeaders(),body:JSON.stringify({code:claimCode})}));setClaimCode("");setMessage("Referral code claimed. Rewards unlock after your first eligible completed buy order.");await loadReferrals();}catch(reason){setMessage(reason instanceof Error?reason.message:"Referral claim failed.");}finally{setSaving(false);}}
  ...
  async function addCharacter() {
    ...
    setMessage("");
    try {
      const character = await addSavedCharacter(
        characterName,
        Number(preferredWorld) || undefined,
      );
      setCharacters((current) => [...current, character]);
      setCharacterName("");
      setPreferredWorld("");
      setMessage("OSRS character saved.");
    } catch (reason) {
      setMessage(
        reason instanceof Error ? reason.message : "Could not save character.",
      );
    } finally {
      setSaving(false);
    }
      ...
      <main className="mx-auto min-h-[700px] max-w-3xl px-6 py-20">
        <div className="rounded-3xl border border-white/10 bg-white/[.025] p-8 sm:p-12">
          ...
          <p className="mt-5 leading-7 text-white/45">
            {error ||
              "Your RuneVault account is required to view private order history."}
          </p>
          <Link
            href="/auth"
  ...
  return (
    <main className="mx-auto min-h-[760px] max-w-7xl px-6 py-14 sm:py-20">
      <section className="flex flex-col justify-between gap-8 lg:flex-row lg:items-end">
        <div>
          ...
          <p className="mt-4 max-w-2xl leading-7 text-white/45">
            Review your OSRS gold orders, totals, references, and current
      ...
      {message && (
        <p
          className="mt-6 rounded-xl border border-white/10 bg-white/[.025] p-4 text-sm text-white/60"
          role="status"
        >
          {message}
        </p>
      ...
      <section className="mt-10 grid gap-6 lg:grid-cols-2">
        <article className="rounded-3xl border border-white/10 bg-white/[.025] p-6 sm:p-8">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <label className="text-sm font-bold text-white/45">
              Name
              <input
                value={fullName}
            ...
            <label className="text-sm font-bold text-white/45">
              ...
              <input
                value={runescapeName}
                maxLength={12}
                onChange={(event) => setRunescapeName(event.target.value)}
                className="mt-2 w-full rounded-xl border border-white/10 bg-black/15 p-3 text-white"
              />
            </label>
            <label className="text-sm font-bold text-white/45">
            ...
            <label className="text-sm font-bold text-white/45">
              ...
              <select
                ...
                onChange={(event) =>
                  setPreferredPayment(
                    event.target.value as typeof preferredPayment,
                  )
                }
          ...
          <label className="mt-5 flex items-center gap-3 text-sm text-white/55">
            ...
            />
            Email order and payment updates
          </label>
          <button
            type="button"
            disabled={saving}
            onClick={() => void saveProfile()}
            className="mt-5 rounded-xl bg-amber-400 px-5 py-3 font-black text-black disabled:opacity-50"
          >
            Save profile
          </button>
        </article>
        ...
        <article className="rounded-3xl border border-white/10 bg-white/[.025] p-6 sm:p-8">
          ...
          <div className="mt-5 flex flex-col gap-3 sm:flex-row">
            <input
              ...
              className="min-h-12 flex-1 rounded-xl border border-white/10 bg-black/15 px-4"
            />
            <input
              ...
              max="999"
              placeholder="World"
              className="min-h-12 w-full rounded-xl border border-white/10 bg-black/15 px-4 sm:w-28"
            />
            <button
              type="button"
              disabled={saving || !characterName.trim()}
              onClick={() => void addCharacter()}
              className="rounded-xl border border-amber-300/30 px-4 font-black text-amber-300 disabled:opacity-50"
            >
              Add
            </button>
          </div>
          <div className="mt-4 space-y-2">
            {characters.map((character) => (
              ...
              >
                <span>
                  <b>{character.name}</b>
                  {character.preferred_world ? (
                    <small className="ml-2 text-white/35">
                      World {character.preferred_world}
                    </small>
                  ) : null}
                </span>
                <button
                  type="button"
                  onClick={() =>
                    void deleteSavedCharacter(character.id)
                      .then(() =>
      ...
      <section className="mt-10 grid gap-4 lg:grid-cols-[1.5fr_.5fr]">
        <div className="rounded-3xl border border-white/10 bg-white/[.025] p-6 sm:p-8">
          <div className="flex items-center justify-between gap-4">
            ...
            <Link
              href="/orders"
              className="text-sm font-bold text-white/45 hover:text-amber-300"
            >
              Track an order
            </Link>
          </div>
        ...
        <aside className="space-y-4">
          ...
          <article className="rounded-3xl border border-white/10 bg-white/[.025] p-6">
            ...
            <p className="mt-2 text-sm leading-6 text-white/40">
              Only the signed-in customer and authorized staff can access these
              database orders.
            </p>
          </article>
        </aside>
      ...
      <section className="mt-10 rounded-3xl border border-white/10 bg-white/[.025] p-6 sm:p-8">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-3">
          <div className="rounded-2xl border border-white/10 p-5">
            <p className="text-sm text-white/40">Available points</p>
            <p className="mt-2 text-3xl font-black">
              {rewards?.points_balance ?? 0}
            </p>
          </div>
          <div className="rounded-2xl border border-white/10 p-5">
            <p className="text-sm text-white/40">Lifetime points</p>
            <p className="mt-2 text-3xl font-black">
              {rewards?.lifetime_points ?? 0}
            </p>
          </div>
          <div className="rounded-2xl border border-white/10 p-5">
            <p className="text-sm text-white/40">VIP tier</p>
            <p className="mt-2 text-3xl font-black capitalize">
              {rewards?.vip_tier ?? "standard"}
            </p>
          </div>
        </div>
        <div className="mt-5 space-y-2">
          <div className="mb-5 grid gap-4 rounded-2xl border border-white/10 p-5 lg:grid-cols-2">
            <div><p className="text-sm font-bold text-white/55">Your referral code</p><div className="mt-2 flex gap-2"><code className="flex-1 rounded-xl bg-black/20 p-3 font-black text-amber-200">{referralCode||"Migration required"}</code><button type="button" disabled={!referralCode} onClick={()=>void navigator.clipboard.writeText(referralCode)} className="rounded-xl border border-white/10 px-4 font-black disabled:opacity-40">Copy</button></div><p className="mt-2 text-xs text-white/35">Both accounts receive 100 points after the referred customer completes an eligible $20+ buy order. Self-referrals and late claims are rejected.</p></div>
            <div><label className="text-sm font-bold text-white/55">Claim a referral code<input value={claimCode} onChange={event=>setClaimCode(event.target.value.toUpperCase().replace(/[^A-Z0-9]/g,"").slice(0,20))} placeholder="RV..." className="mt-2 w-full rounded-xl border border-white/10 bg-black/15 p-3 uppercase"/></label><button type="button" disabled={saving||claimCode.length<8} onClick={()=>void claimReferral()} className="mt-2 w-full rounded-xl bg-amber-400 p-3 font-black text-black disabled:opacity-40">Claim code</button><p className="mt-2 text-xs text-white/35">Referral records: {referrals.length} · Rewarded: {referrals.filter(item=>item.status==="rewarded").length}</p></div>
          </div>
          {rewardHistory.slice(0, 10).map((item) => (
            ...
            >
              ...
              <b
                className={
                  item.points >= 0 ? "text-emerald-300" : "text-rose-300"
                }
              >
                {item.points > 0 ? "+" : ""}
                {item.points}
          ...
          {!rewardHistory.length && (
            <p className="text-sm text-white/35">
              Points are awarded once eligible buy orders are completed.
            </p>
          )}
        </div>
      </section>
