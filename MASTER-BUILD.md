...
- [ ] One-page checkout, cart, validation, discounts, terms, inventory/price verification, cancellation/retry, and mobile tests. (One-page responsive checkout, persistent validated multi-item buy/sell cart with editable quantities/removal/clear and per-item secure checkout, server-delivered authoritative pricing, stable request idempotency, coupon input/atomic rules, terms evidence, inventory check/reservation attempt, and narrowly scoped owner-cancellation RPC implemented. Live cart/mobile and authenticated order creation were verified; production cancellation migration and post-fix authenticated retest remain.)
...
- [ ] Price history, schedules, promotions, tiers, pause controls, and server-side totals. (Additive schema, history trigger, public reads, permission-protected scheduling/tier admin controls with audit writes, deterministic newest-schedule/highest-tier resolution, matching quote/checkout previews, migration-safe fallback, and server-authoritative order totals implemented; production migration and authenticated live tests remain.)
...
- [ ] Customer-isolated timelines, payment/verification/assignment/delivery/payout status, instructions, invoices, reorder, and linked support. (Stable private `/orders/[reference]` pages, timeline data/fallback, full status panels, printable receipt, reorder, retry-payment, and linked-support actions are implemented; migration/live authenticated tests remain.)
- [ ] Section 9 live tests and deployment.

## Section 10 — live chat, tickets, and help center

- [ ] Realtime guest/customer chat, staff inbox, presence, assignments, history, files, moderation, notifications, and transcripts. (Floating mobile/desktop widget, signed-in and token-isolated guest chat, order linking, Realtime message refresh with polling fallback, staff/customer presence and bidirectional typing indicators, private attachment upload/list/signed-download UI, staff inbox, search, assignment, replies/internal notes, blocking, ratings API, availability messaging, durable mutation limits, schema/Realtime/RLS implemented; customer reply notification and live migration/storage tests remain.)
